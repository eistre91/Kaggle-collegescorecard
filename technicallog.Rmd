...
cor(relative$relative, state_earnings$median_earnings)

cor(trim$admission_rate.overall, trim$X10_yrs_after_entry.median, use="complete.obs")
...
#had some outlier years
filter(trim, name == "California State University-San Bernardino")$admission_rate.overall

#there's a section that's really weird in the above plot where admission rates are really low and scores are lower than might be expected.
#in the case of california state university-san bernardino, there seems to have been a sudden drop of admittance rate which returned to normal after 3 years
    #recession caused?
```
...
Paying out-of-state vs in-state tuition can be an important factor.

```{r}
earnings_debt$tuition.out_of_state
earnings_debt$tuition.in_state
earnings_debt$X10_yrs_after_entry.median
earnings_debt$median_debt.completers.overall
earnings_debt$earnings_debt_ratio
...
The debt relationship disappears for the mid students.
...
#for regular_test_scores matrix
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
#university of phoenix incorrectly reported their earnings?
#rolled it up to aggregate for earnings
